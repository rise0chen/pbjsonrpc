...
impl DescriptorSet {
    ...
    pub fn register_file_descriptor(&mut self, file: FileDescriptorProto) {
        ...
        for descriptor in file.service {
            self.register_service(&path, descriptor)
        }
    }
    ...
    fn register_service(&mut self, path: &TypePath, descriptor: ServiceDescriptorProto) {
        let name = TypeName::new(descriptor.name.expect("expected name"));
        let child_path = path.child(name);
        self.register_descriptor(
            child_path.clone(),
...
impl DescriptorSet {
    pub fn get(&self, type_name: &str) -> Option<(&TypePath, &Descriptor)> {
        self.iter()
