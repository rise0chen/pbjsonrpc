...
use crate::descriptor::{
    Descriptor, DescriptorSet, MessageDescriptor, ServiceDescriptor, Syntax, TypeName, TypePath,
};
...
impl Method {
    pub fn rust_method_name(&self) -> String {
        use heck::ToSnakeCase;
        escape_ident(self.name.to_snake_case())
    }
}
...
pub fn resolve_service(
    ...
    for method in &service.methods {
        let is_stream = method.server_streaming();
        let input = descriptors.get(method.input_type()).unwrap().1;
        let output = descriptors.get(method.output_type()).unwrap().1;
        let input = if let Descriptor::Message(input) = input {
            resolve_message(descriptors, input).unwrap()
        } else {
            unreachable!()
        };
        let output = if let Descriptor::Message(output) = output {
            resolve_message(descriptors, output).unwrap()
        } else {
            unreachable!()
        };
...
fn resolve_type(descriptors: &DescriptorSet, type_name: &str) -> FieldType {
    ...
    let maybe_descriptor = descriptors.get(type_name);
    ...
    match maybe_descriptor {
        ...
        _ => panic!("failed to resolve type: {}", type_name),
    }
}
